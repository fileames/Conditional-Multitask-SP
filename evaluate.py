...
def get_dataloaders(tasks, batch_size, setting = "nyu", task = None):
    ...
    if setting == "taskonomy":
        ...
        dataloader = DataLoader(subset_dataset_test, batch_size=batch_size, shuffle=False)
        ...
        for j in range(len(img)):
            if len(logits) == 1:
                ...
                if task == "segmentation":
                    ...
                    k = disp2meters(torch.nn.functional.sigmoid(logits[0][j])*65535 + 1)
                ...
                k2 = disp2meters(torch.nn.functional.sigmoid(logits[1][c])*65535 + 1)
...
def main():
    ...
    dataloader = get_dataloaders(tasks, 16, config["setting"], config["task"] if "task" in config.keys() else None)
    ...
    model = SwinTransformer(img_size=224,
                        ...
                        use_conditional_layer = config["use_conditional_layer_norm"] if "use_conditional_layer_norm" in config.keys() else None)
                                                   
    model = load_model(model, config["model_path"], device)
    
    print("Evaluating...")                                              
    evaluate(model, dataloader, device, task = config["task"] if "task" in config.keys() else None )
       
    print("Saving Images...")  
    save_images(model, dataloader, device, num_images = config["num_generated_images"],  task = config["task"] if "task" in config.keys() else None, setting =  config["setting"])                                            

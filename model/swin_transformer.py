...
import math
from model.conditional_modules import CBDA, ConditionalBottleNeck, ConditionalLayerNorm
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., task_configs=None):
        ...
        if task_configs is not None:
            ...
            # hidden size d, max_seg_length L, num_blocks N
    ...
    def forward(self, x, task_embedding=None, mask=None):  # TODO: task embedding ver
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 fused_window_process=False, task_configs=None, hidden_size=343, use_conditional_layer=False):
        super().__init__()
        self.dim = dim
        ...
        #print(task_configs)
        self.use_conditional_layer = use_conditional_layer
        
        
        if task_configs is None or (not self.use_conditional_layer):
            self.norm1 = norm_layer(dim)
        else:
            self.norm1 = ConditionalLayerNorm(dim, hidden_size)
        ...
        if task_configs is None or (not self.use_conditional_layer):
            ...
            self.norm2 = ConditionalLayerNorm(dim, hidden_size)
    ...
    def forward(self, x, task_embedding=None, task_id = None):
        ...
        if self.task_configs is None or (not self.use_conditional_layer):
            x = self.norm1(x)
        else:
            x = self.norm1(x, task_embedding, task_id)
        ...
        if self.task_configs is None or (not self.use_conditional_layer):
            ...
            x = x + self.drop_path(self.mlp(self.norm2(x, task_embedding, task_id)))
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 fused_window_process=False, task_configs=None, conditioned_blocks=[0], adapter=False, hidden_size=343, use_conditional_layer =False):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                ...
                                use_conditional_layer = use_conditional_layer)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, upsample=None, use_checkpoint=False, use_conditional_layer=False):
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 conditioned_blocks = [[],[],[12],[]], adapter = False, use_conditional_layer = False,
                 **kwargs):
        super().__init__()
        ...
        if hidden_size is None:
            self.hidden_size = self.max_seq_length * \
                window_size  # 49 * 7 , N = 7 olması için
        else:
    ...
    def forward(self, x, task_id):
        ...
        return logits, unique_task_ids_list 
